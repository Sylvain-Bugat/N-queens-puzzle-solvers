 ...
 * 
 */
public final class BackTrackingNQueensSolverRevertedArray extends GenericNQueensSolver {

	/** Chessboard represented by a 2 dimensional array. */
	private int[][] chessboard;

	public BackTrackingNQueensSolverRevertedArray(final int chessboardSizeArg, final boolean printSolutionArg) {
	...
	private boolean checkValidChessboard() {
		...
		for (int diagonal = 0; diagonal < chessboardSize * 2 - 1; diagonal++) {
			...
			for (int y = 0; y < chessboardSize; y++) {
				...
				if (x >= 0 && x < chessboardSize && 1 == chessboard[y][x]) {
